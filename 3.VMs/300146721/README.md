...
### 1.J'installe Active Directory
```powershell
Rename-Computer -NewName "DC300146721" -Restart
```
### 2.J'installe le rôle AD DS
```powershell
Install-WindowsFeature AD-Domain-Services -IncludeManagementTools
```
### 3.J'ai Crée une nouvelle forêt
```powershell
Install-ADDSForest `
    -DomainName "DC300146721-00.local" `
    -DomainNetbiosName "DC300146721-00" `
    -InstallDns:$true `
    -SafeModeAdministratorPassword (ConvertTo-SecureString "MotDePasseDSRM123!" -AsPlainText -Force) `
    -Force
```
### 4.Je vérifie que mon contrôleur de donnaime est bien installé
```powershell
Get-ADDomain
```
```txt
